            ...
            Bundle savedInstanceState) 
    {
       ...
       imei = AppUtils.getToken(mContext);
		...
		int PersonNodeType=sharedPref.getInt("SalesmanNodeType",0);
	      int check=dbengine.fnCountToDisplayDailySummaryDetailsStoreSKUWise(1,flgReportFromTmpOrPermanent,PersonNodeID,PersonNodeType);
	     // dbengine.close();
			...
			{
				//dbengine.open();
				hmapAll_details=dbengine.fnGetDailySummaryDetailsStoreSKUWiseLevel(1,flgReportFromTmpOrPermanent,PersonNodeID,PersonNodeType);
				//dbengine.close();
	            intializeFields();
			}

...
import com.astix.rajtraderssfasmasales.model.TblPDARouteChangeApproval;
   ...
   /* @BindView(R.id.ll_optContainer)
    ...
    private void sendOtp()
    {
        if(isOnline()) {
            ...
            data.setOldRouteNodeID(Integer.parseInt(mDataSource.GetActiveRouteID(CommonInfo.CoverageAreaNodeID, CommonInfo.CoverageAreaNodeType)));
    ...
    private void sendConfirmationToServer(){
        if(isOnline()) {
            ...
            data.setSelectedRouteChageComment(selectedRouteChageComment);
            data.setOTP(generatedOTPForRetailer);
            data.setOTPTemplate(generatedOTPForRetailer);
            data.setFlgStep(2);

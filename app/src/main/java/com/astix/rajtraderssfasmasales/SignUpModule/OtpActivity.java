package com.astix.rajtraderssfasmasales.SignUpModule;
...
import com.astix.rajtraderssfasmasales.SplashScreen;
...
public class OtpActivity extends BaseActivity implements View.OnTouchListener{
   ...
   /* @BindView(R.id.et_one)
    ...
    void showDialogRetry() {
        ...
        alertDialog.setMessage("There is no internet connection. Please enable your internet and retry");
    ...
    private void sendOtp()
    {
       /* if(isOnline())
        ...
        if(isOnline())
        {
            ...
            progressHUD.show();
            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
            Call call = apiService.sendOtp(mobileNum, generatedOTP.toString()+" is the OTP for validation of your contact number for ASM Application.Please do not share the OTP with anyone.");
            call.enqueue(new Callback() {
                @Override
                public void onResponse(Call call, Response response) {
                    progressHUD.dismiss();

            ...
            Bundle savedInstanceState) 
    {
       rootView = inflater.inflate(R.layout.store_and_sku_wise, container, false);
		...
		int PersonNodeType=sharedPref.getInt("SalesmanNodeType",0);
	      int check=dbengine.fnCountToDisplayDailySummaryDetailsStoreSKUWise(5,flgReportFromTmpOrPermanent,PersonNodeID,PersonNodeType);
	     // dbengine.close();
			...
			{
				//dbengine.open();
				hmapAll_details=dbengine.fnGetDailySummaryDetailsStoreSKUWiseLevel(5,flgReportFromTmpOrPermanent,PersonNodeID,PersonNodeType);
				//dbengine.close();
	            intializeFields();
			}

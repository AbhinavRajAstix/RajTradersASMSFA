...
import com.astix.rajtraderssfasmasales.adapter.LastVisitHistoryAdapter;
import com.astix.rajtraderssfasmasales.merchandizing.PicBeforeMerchandising;
...
public class LastVisitDetails extends BaseActivity {
    ...
    TextView btnViewDetailsLastVisit;
                ...
                if (flgOrderType == 0) {
                    if( CommonInfo.hmapStoreLocationDetails!=null &&  CommonInfo.hmapStoreLocationDetails.size()>0) {
                        mDataSource.fnInsertOrUpdate_tblStoreVisitMstr(StoreVisitCode, storeID, 1, getDateInMonthTextFormat(), CommonInfo.hmapStoreLocationDetails.get("ActualLatitude"), CommonInfo.hmapStoreLocationDetails.get("ActualLongitude"), getDateAndTimeInMilliSecond(), getDateAndTimeInMilliSecond(), getDateAndTimeInMilliSecond(), getDateAndTimeInMilliSecond(), CommonInfo.hmapStoreLocationDetails.get("LocProvider"), CommonInfo.hmapStoreLocationDetails.get("Accuracy"), passdLevel, 0, 0, 0, 0, 0, 0, Integer.parseInt(CommonInfo.hmapStoreLocationDetails.get("flgLocationServicesOnOff")), Integer.parseInt(CommonInfo.hmapStoreLocationDetails.get("flgGPSOnOff")), Integer.parseInt(CommonInfo.hmapStoreLocationDetails.get("flgNetworkOnOff")), Integer.parseInt(CommonInfo.hmapStoreLocationDetails.get("flgFusedOnOff")), Integer.parseInt(CommonInfo.hmapStoreLocationDetails.get("flgInternetOnOffWhileLocationTracking")), 0, 0, 0,
                                flgVisitCollectionMarkedStatus, AddNewStore_DynamicSectionWise.address, AddNewStore_DynamicSectionWise.city, AddNewStore_DynamicSectionWise.pincode, AddNewStore_DynamicSectionWise.state, mrouteID, mrouteType, imei, flgOrderType, inRange, isLocationRadioChecked,StoreSelection.JointVisitId,FLGJOINTVISIT,VisitType,flgStoreInComingOrTelephonic);
...
//int i=1/0;
        ...
        btnViewDetailsLastVisit=findViewById(R.id.btnViewDetailsLastVisit);
        ...
        SpannableString content1 = new SpannableString(btnViewDetailsLastVisit.getText());
        content1.setSpan(new UnderlineSpan(), 0, content1.length(), 0);



        btnViewDetailsLastVisit.setText(content1);
        btnViewDetailsLastVisit.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                OpenPopUpDialog();
            }
        ...
        if(mDataSource.fncheckStoreIsNewOrOld(storeID)==1)
        {
            btnViewDetailsLastVisit.setVisibility(View.GONE);
        }
...
*/
    ...
    private void startActualVisit() {
        if(VisitType==3)
        {
            ...
            args.putString(AppUtils.StoreId, storeID);
            ...
            Intent mainIntent = new Intent(LastVisitDetails.this,DynamicQuestionsFragment.class);
...
*/
    ...
    protected void OpenPopUpDialog() {
        dialog = new Dialog(LastVisitDetails.this);
        ...
        String ImageUrl = CommonInfo.WebUrlLinkforLastVisitDetails.trim();

        ImageUrl = ImageUrl + "?StoreID=" +storeID;//+"?ContactNo="+ContactNo+"&userName="+userName+"&ApplicationID="+ApplicationID;
        ...
        try {
            ...
            web_view_imageStore.setWebViewClient(new WebViewClient());

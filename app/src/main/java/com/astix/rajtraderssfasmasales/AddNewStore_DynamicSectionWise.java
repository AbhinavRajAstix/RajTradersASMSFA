...
public class AddNewStore_DynamicSectionWise extends BaseActivity implements LocationInterface, SearchListCommunicator, OnMapReadyCallback, CategoryCommunicatorCityState, Camera.PictureCallback,InterfaceRetrofitNewStoreNumberValidation {
    public static int mFeedbackType = -1;
    public static String JOINTVISITID = "NA";
    private int flgJointVisit = 1;
...
//            isGPSok = false;
        ...
        JOINTVISITID = extras.getStringExtra(AppUtils.JOINTVISITID);//Fetch the Joint Visit Code
        activityFrom = extras.getStringExtra("activityFrom");
        mFeedbackType = extras.getIntExtra(AppUtils.FeedbackType,0);
        flgJointVisit = extras.getIntExtra(AppUtils.FLGJOINTVISIT,0);
...
*/
    ...
    private void fillHmapData() {
     ...
     /*   hmapQuesIdValues = mDataSource.fnGetQuestionMstr(sPref);    //
        ...
        hmapQuesIdValues = mDataSource.fnGetQuestionMstr(sPref,mFeedbackType);
        hmapQuesGropKeySection = mDataSource.fnGetQuestionMstrKey(mFeedbackType);
        hmapGroupId_Desc = mDataSource.getGroupDescription(mFeedbackType);
        hmapSctnId_GrpId = mDataSource.fnGetGroupIdMpdWdSectionId(mFeedbackType);
        hmapDpndtQustGrpId = mDataSource.fnGetDependentQuestionMstr();
        hmapSection_key = mDataSource.fnGetSection_Key(mFeedbackType);
       ...
       // QuestIDForOutChannel = mDataSource.fnGetQuestIDForOutChannelFromQuestionMstr(); //
        ...
        QuestIDForOutChannel = mDataSource.fnGetQuestIDForOutChannelFromQuestionMstr(mFeedbackType);
...
*/
    ...
    public void  getStoreVisitCode()
    {
      ...
      /*  mDataSource.fnInsertOrUpdate_tblStoreVisitMstr(StoreVisitCode,selStoreID,3,getDateInMonthTextFormat(),LattitudeFromLauncher,LongitudeFromLauncher,getDateAndTimeInMilliSecond(),getDateAndTimeInMilliSecond(),getDateAndTimeInMilliSecond(),getDateAndTimeInMilliSecond(),ProviderFromLauncher,AccuracyFromLauncher,passdLevel,0,0,1,0,0,0,flgLocationServicesOnOff,flgGPSOnOff,flgNetworkOnOff,
                ...
                0,flgVisitCollectionMarkedStatus,address,city,pincode,state,rID,mrouteType, imei,1,1,1);//Last Parameter here 1 is flgOrderType means Actual Visit
        mDataSource.fnInsertOrUpdate_tblStoreOrderVisitDayActivity(StoreVisitCode,selStoreID,StoreVisitCode,getDateInMonthTextFormat(),3
                ,getDateAndTimeInMilliSecond(),getDateAndTimeInMilliSecond(), imei);*/
    ...
    public String genStoreVisitCode()
    {
        ...
        SimpleDateFormat df = new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss",Locale.ENGLISH);
        String VisitStartTS = TimeUtils.getNetworkDateTime(AddNewStore_DynamicSectionWise.this, TimeUtils.DATE_TIME_FORMAT);;
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if (!LattitudeFromLauncher.equals("NA") && !LattitudeFromLauncher.equals("0.0")) {
         ...
         /*   googleMap.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener() {
            ...
            if (hmapOutletListForNearUpdated != null && hmapOutletListForNearUpdated.size()>0) {
                //mDataSource.open();
...
//            if(countSubmitClicked==1)
    ...
    public void successNewStoreNumberValidation(TblDuplicateContact tblDuplicateContact,String ServerNumber) {
        ...
        {
            mDataSource.fnsaveOutletQuestAnsMstrSectionWise(hmapStoreQuestAnsNew, 0, selStoreID, JOINTVISITID, mFeedbackType);
